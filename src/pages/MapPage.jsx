...
import { MAP_CENTER, MAP_ZOOM, TILE_URL, TILE_ATTRIBUTION } from '../constants/index.js';
import { nearestFreeVehicle, randomStep, nowTime, etaMinutes } from '../utils/geo.js';
...
export default function MapPage({
  ...
  useEffect(() => {
    ...
    const m = L.map(mapElRef.current, {
      center: MAP_CENTER, zoom: MAP_ZOOM, zoomControl: false,
    });
  ...
  useEffect(() => {
        ...
        if (exists) {
          return prev.map(v =>
            String(v.id) === String(d.driverId)
              ? { ...v, lat: d.lat ?? v.lat, lng: d.lng ?? v.lng, status: d.status ?? v.status }
              : v
          );
        }
        return [...prev, {
          id: d.driverId, name: d.name || d.driverId, driver: d.name || d.driverId,
          phone: '', lat: d.lat || MAP_CENTER[0], lng: d.lng || MAP_CENTER[1],
          status: d.status || 'slobodan', live: true,
        }];
      });
    });
    ...
    socket.on('driver:removed', ({ driverId }) => {
      setVehicles(prev =>
        prev.map(v => String(v.id) === String(driverId) ? { ...v, status: 'offline' } : v)
      );
    });
  ...
  useEffect(() => {
    const id = setInterval(() => {
      ...
      setVehicles(prev =>
        prev.map(v => v.status === 'offline' ? v : { ...v, ...randomStep(v.lat, v.lng) })
      );
    }, 2500);
  ...
  const handlePlaceOrder = (form) => {
    ...
    const order = {
      id: Date.now(), ...form,
      vehicle: nearest.name, driver: nearest.driver,
      driverPhone: nearest.phone, status: 'Aktivna',
      eta: eta + ' min', time: nowTime(),
    };

    setOrders(prev => [order, ...prev]);
    setVehicles(prev => prev.map(v => v.id === nearest.id ? { ...v, status: 'zauzet' } : v));
    push(`✅ ${nearest.driver} je na putu! Dolazak za ~${eta} min.`, 'success');

    setTimeout(() => {
      setOrders(prev => prev.map(o => o.id === order.id ? { ...o, status: 'Završena' } : o));
      setVehicles(prev => prev.map(v => v.id === nearest.id ? { ...v, status: 'slobodan' } : v));
    }, eta * 6_000);
  };
  ...
  return (
    <div className={styles.layout}>
      ...
      <div className={styles.mapWrap}>
        ...
        <button className={styles.toggle} onClick={() => setSideOpen(p => !p)}>
          {sideOpen ? '◀' : '▶'}
        </button>
      </div>
...
function mergeDrivers(existing, list) {
  ...
  list.forEach(d => {
    ...
    if (idx >= 0) {
      updated[idx] = { ...updated[idx], lat: d.lat ?? updated[idx].lat, lng: d.lng ?? updated[idx].lng, status: d.status ?? updated[idx].status, live: true };
    } else {
      updated.push({ id: d.driverId, name: d.name, driver: d.name, phone: '', lat: d.lat, lng: d.lng, status: d.status, live: true });
    }
  });
  return updated;
}

const STATUS_COLOR = { slobodan: '#16a34a', zauzet: '#dc2626', offline: '#6b7280' };
...
function buildIcon(vehicle, isSelected) {
  ...
  const liveTag = vehicle.live
    ? `<span style="position:absolute;top:-4px;right:-4px;width:9px;height:9px;
        border-radius:50%;background:#ef4444;border:2px solid #fff;display:block"></span>`
    : '';
  ...
  return L.divIcon({ html, className: '', iconSize: [80, 58], iconAnchor: [40, 28] });
}
